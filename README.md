...
| Data Analysis<br>Data-preprocessing<br>모델 학습<br>(Logistic Regression)<br>프론트<br>Result Comfirm | Data Analysis<br>Data-preprocessing<br>모델 학습<br>(Random Forest classification)<br>프론트<br>Label Flipping | Data Analysis<br>모델 학습<br>(Random Forest classification)<br>Conclusion results<br>README 작성 | Data Analysis<br>모델 학습<br>(Logistic Regression)<br>Conclusion results<br>README 작성 | Data Analysis<br>모델 학습<br>(Logistic Regression)<br>(XGBoost)<br>(LightGBM) |   
# :computer: 프로젝트 개요
- **프로젝트 목적**  
...
# 관리자 페이지 개발
ML 모델을 사용한 데이터 분석 외에 추가로 더 얻을 수 있는 정보나 실제 서비스에 활용할 수 있는 방안이 있을지 고민했다.

## 모델의 FP, FN 데이터들의 레이블 플리핑을 활용하여 각 고객별 특성 파악
###    
- **레이블 플리핑**   
...
4. 과대적합된 트리 모델의 생성, 데이터 별로 임의의 노드 경로를 바꿨을 때 반대의 타겟값이 나오게 되는 노드를 탐색한다.

![image](https://github.com/user-attachments/assets/b10fa96a-2d93-43bb-9a70-57d934f959a3)
...
이탈 예상, 위험 고객으로 판단한 Logistic 모델의 FP 예측 데이터들에 대해 Positive 로 판단한 이유를 탐색할 수 있었다.
예측 결과가 바뀌는 노드의 분기 조건을 만족시키도록 고객에게 피드백을 줌으로써 이탈 위험도를 낮춰줄 수 있다. 
...
1. for문 반복 관련 오류    
   ...
   해결: sample_weights 따로 적용했다.
###    
2. for문 반복 관련 오류2   
   for문을 통해 기준값(trashold)과 가중치(weights) 동시 적용시 학습 문제 trashold 와 weights 중에서 1가지만 학습이 진행된다.   
   ...
   해결: 각각 한번씩 반복하며 학습을 진행한다.   
###   
3. 소수점 오류   
   그래프 title에 보이는 소수값이 틔는 현상이 존재한다.
   해결 : " *0.1 " => " /10 " 변경   
###      
# 📌느낀점   
...
김지훈 : 익숙하지 않은 접근법이었을 텐데도 팀원들이 믿고 같이 진행해줘서 온전히 제 업무에 집중해서 진행할 수 있었습니다. 
